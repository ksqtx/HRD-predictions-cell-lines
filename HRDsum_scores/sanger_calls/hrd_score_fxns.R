...
annotate_arms <- function(cn_df, chr_info) {
  cn_df %>%
    ...
    dplyr::select(!c(chr_length, dplyr::starts_with('cent')))
}

# Split segments that span the centromere into p and q portions (for calculating LSTs)
split_cent_segments <- function(annotated_cn_df, chr_info) {
  annotated_cn_df %>%
    dplyr::left_join(chr_info, by = 'chr') %>%
...
# Remove segments shorter than specified cut-off
discard_short_segments <- function(arm_df, length_cutoff) {
  arm_df %>%
    dplyr::rowwise() %>%
    dplyr::mutate(length = end - start) %>%
    as.data.frame() %>%
    dplyr::filter(length >= length_cutoff) %>%
    dplyr::select(!length)
}
...
collapse_segments <- function(arm_df) {
  arm_df %>%
    ...
    dplyr::mutate(start = min(start),
                  ...
                  n_snps = sum(n_snps)) %>%
    as.data.frame() %>%
    dplyr::distinct(start, end, n_snps, .keep_all = TRUE) %>%
